...
def init_seller_province():
    province = [
        '北京', '上海', '天津', '湖南', '重庆', '云南', '河南', '海南', '台湾',
        '甘肃', '四川', '江西', '广东', '广西', '黑龙江', '内蒙古', '新疆', '西藏', 
        '河北', '湖北', '辽宁', '吉林', '浙江', '山西', '陕西', '青海', '山东',
        '福建', '江苏']
    for i in range(500):
        seller = User.objects.create(
            username='sellerhead'+str(i), user_type=USER_TYPE['seller'])
        seller.set_password('123')
        seller.save()
        SellerInfo.objects.create(
            seller=seller, city=province[random.randint(0, len(province)-1)],
            price=24.5)
    for i in range(50):
        seller = User.objects.create(
            username='sellerdirect'+str(i), user_type=USER_TYPE['seller'])
        seller.set_password('123')
        seller.save()
        for i in range(4):
            SellerInfo.objects.create(
                seller=seller, city=province[i],
                price=24.5)
